...
class AccessibilityValidator:
    ...
    def validate_file(self, file_path: Path) -> list[dict]:
        ...
        for table in tables:
            if self._has_empty_first_cell(table["header"]):
                file_issues.append(
                    {
                        "line": table["line_start"] + 1,
                        "type": "empty_first_cell",
                        "message": (
                            "Table header has an empty first cell (use | Header | not | | Header |)"
                        ),
                        "file": file_path.name,
                        "fixable": True,
                    }
                )
            for row_line in range(table["line_start"] + 2, table["line_end"] + 1):
                row = lines[row_line] if row_line < len(lines) else ""
                if self._has_empty_first_cell(row):
                    file_issues.append(
                        {
                            "line": row_line + 1,
                            "type": "empty_first_cell",
                            "message": (
                                "Table row has an empty first cell "
                                "(remove leading | | or fill the cell)"
                            ),
                            "file": file_path.name,
                            "fixable": True,
                        }
            ...
            file_issues.append(
                {
                    ...
                    "message": (
                        'GIF present but no "Screen Reader: [Topic]" workflow for blind users'
                    ),
                    "file": file_path.name,
                    "fixable": False,
                }
        ...
        accessibility_section = re.search(
            r"#+.*(accessibility|Barrierefreiheit)", content, re.IGNORECASE
        )
            ...
            file_issues.append(
                {
                    ...
                    "message": (
                        "Accessibility section should mention high contrast/dark mode support"
                    ),
                    "file": file_path.name,
                    "fixable": False,
                }
        ...
        while i < len(lines):
            if "|" in lines[i]:
                ...
                if next_is_sep:
                    ...
                    j = i + 2
                    while (
                        j < len(lines) and "|" in lines[j] and not lines[j].strip().startswith("#")
                    ):
                        j += 1
    ...
    def _has_empty_first_cell(self, row: str) -> bool:
        """Check if a table row has an empty first cell (| | at start)."""
        stripped = row.strip()
        if not stripped.startswith("|"):
            return False
        cells = [c.strip() for c in stripped.split("|")]
        # cells[0] is empty (before first |), cells[1] is the first real cell
        return len(cells) >= 2 and cells[1] == ""
    ...
    def fix_file(self, file_path: Path) -> int:
        ...
        # Table header fixing is handled by scripts/fix_table_bugs.py
        # The validator only reports empty-first-cell issues; it does not
        # auto-fix them (column-count matching is non-trivial).
