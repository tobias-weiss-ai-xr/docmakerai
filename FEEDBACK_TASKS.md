...
## Wave 6 — Regression-Guards (nach Wave 1+2)
...
Legende: `DONE-B1` Tabellen-Header · `DONE-B2` Accessibility-Übersetzung · `DONE-B3` obere Leiste statt Seitenleiste · `DONE-B4` Rechtsklick entfernt · `DONE-B5` Auto-Antwort-Umbenennung · `DONE-B7` Reviewer-Phrasen · `DONE-B8` Index-Tabelle · `DONE` Lob/keine Aktion · `REF` Querverweis · `T*` offene Task oben
...
| 22 | `sogo-vacation#voraussetzungen` | „Die Abwesenheitsnotiz muss von Ihrem Administrator akt… | T2.14 |
...
| 28 | `sogo-vacation/#schritt-6-speichern` | „Das Sieve-Skript wird auf dem Mail-Server aktiviert.“ … | T1.6 |
| 29 | `sogo-vacation/#test-e-mail-senden` | Sie werden eine Sprachwissenschaftlerin nie sagen hören… | DONE-B5 |
| 30 | `sogo-vacation#auto-antwort-wird-nicht-gesendet` | „Vergewissern Sie sich, dass der Sieve-Server läuft (SO… | T1.6 |
...
| 33 | `sogo-calendar-create-event` | „(…) einschließlich der Festlegung von Zeit, (...)“  Äh… | T1.7 |
...
| 36 | `sogo-calendar-create-event#schritt-3-ereignisdetails-eingeben` | „Feld: Description“  „Eingabefeld“ Ich finde das Eingab… | T2.1 |
| 37 | `sogo-calendar-create-event#schritt-4-teilnehmer-hinzuf%C3%BCgen-optional` | Habe versucht, „Auto-Vervollständigungsvorschlägen“ gef… | T1.8 |
...
| 40 | `sogo-calendar-recurring` | Vorschlag: ich würde dieses gesamte Tutorial in das vor… | T3.1 |
| 41 | `sogo-calendar-recurring#alarm-wird-nicht-ausgel%C3%B6st` | „E-Mail-Alarme erfordern eine serverseitige Konfigurati… | T2.13 |
...
| 45 | `sogo-calendar-views#schritt-2-zwischen-ansichten-wechseln` | „Ansicht: Description“  „Ansicht“ Warum sind da Zahlen … | T2.12 |
| 46 | `sogo-calendar-views#schritt-3-durch-die-zeit-navigieren` | „Durch die Zeit navigieren“  Die Hinweise auf die Relat… | T1.1 |
| 47 | `sogo-calendar-views#ansichtsvergleich` | Als Nutzerin finde ich diese Tabelle überflüssig. Da st… | T3.5 |
| 48 | `sogo-calendar-views#fazit` | Wieder eine Navigation „durch die Zeit“! | T1.1 |
| 49 | `sogo-calendar-views#accessibility` | Same old. | DONE-B2 |
| 50 | `sogo-calendar-edit-delete` | Ich würde dieses Tutorial vor das Kalenderansichten-Tut… | T3.2 |
...
| 53 | `sogo-calendar-edit-delete#schritt-2-ereignis-l%C3%B6schen` | Wieder Punkt 42. Bei „Nur dieses Ereignis löschen“ als … | T1.9 |
| 54 | `sogo-calendar-edit-delete#fehlerbehebung` | Punkt ZWEIUNDVIERZIG. | T1.9 |
...
| 58 | `sogo-calendar-ical#schritt-2-kalendereinstellungen-aufrufen` | Also, in der aktuellen Ansicht ist da kein Zahnrad für … | T2.5 |
| 59 | `sogo-calendar-ical#schritt-3-kalender-exportieren` | Das orientiert sich jetzt aber eher grob an dem, was da… | T2.5 |
| 60 | `sogo-calendar-ical#freigabe-%C3%BCber-ical` | Es kann natürlich sein, dass ich einfach keine Ahnung h… | T2.5 |
| 61 | `sogo-calendar-ical#accessibility` | This is in English again! | DONE-B2 |
| 62 | `sogo-calendar-share` |  | DONE-B1 |
| 63 | `sogo-calendar-share#schritt-1-kalendereinstellungen-%C3%B6ffnen` | Der Kalender ist immer noch nicht über die Seitenleiste… | T2.15 |
| 64 | `sogo-calendar-share#schritt-2-einen-kalender-zum-freigeben-ausw%C3%A4hlen` | Der Persönlich-Kalender heißt allerdings wie das Accoun… | T2.15 |
| 65 | `sogo-calendar-share#schritt-3-einen-benutzer-hinzuf%C3%BCgen` | Den Reiter gibt es nicht – nach „Freigabe…“ gibt man di… | T2.15 |
| 66 | `sogo-calendar-share#schritt-4-berechtigungsstufe-festlegen` | Ausgehend vom aktuellen Interface unter „Freigabe…“ kom… | T2.15 |
| 67 | `sogo-calendar-share#freigabe-%C3%BCber-caldav-erweitert` | Bin zu faul, das zu überprüfen, aber die graue Schrift … | T5.2 |
| 68 | `sogo-calendar-share#freigabe-entfernen-oder-%C3%A4ndern` | Jup, das ist nicht die genaue Abfolge an Schritten, aus… | T2.15 |
...
| 71 | `sogo-calendar-subscribe` | Ich versteh nicht genau, warum das ein eigenes Tutorial… | T3.3 |
...
| 74 | `sogo-calendar-subscribe#schritt-3-feed-url-eingeben` | Hab eben extra einen Feiertagskalender abonniert – so, … | T2.16 |
| 75 | `sogo-calendar-subscribe#schritt-4-sync-optionen-konfigurieren` | Keine Ahnung, wo diese Funktion sein soll; ich hab jede… | T2.16 |
| 76 | `sogo-calendar-subscribe#schritt-5-abonnement-speichern` | Welches Symbol? | T2.16 |
| 77 | `sogo-calendar-subscribe#manuell-aktualisieren` | Pfft. Da aktualisiert sich gar nichts. Aber unter „Eins… | T2.16 |
| 78 | `sogo-calendar-subscribe#abonnement-eigenschaften-bearbeiten` | Nichts davon erreicht man über einen Rechtsklick. | DONE-B4 |
| 79 | `sogo-calendar-subscribe#abonnement-k%C3%BCndigen` | Dito. | T2.16 |
| 80 | `sogo-calendar-subscribe#ung%C3%BCltige-kalender-url` | Hab nix zu meckern, weil ich’s nicht überprüft habe. Wü… | T2.16 |
...
| 85 | `sogo-calendar-freebusy#schritt-2-freigebucht-ansicht-%C3%B6ffnen` | Man muss keine Frei/Gebucht-Schaltfläche anklicken: Nac… | T2.2 |
| 86 | `sogo-calendar-freebusy#schritt-3-einen-kollegen-hinzuf%C3%BCgen` | Hä? Meine Kollegin hab ich doch im zweiten Schritt scho… | T2.2 |
...
| 89 | `sogo-calendar-freebusy#was-andere-sehen` | Glaub ich jetzt einfach unbesehen. Allerdings: Was gena… | T2.8 |
| 90 | `sogo-calendar-freebusy#alle-zeiten-zeigen-keine-daten` | „Kontaktieren Sie ihn oder Ihren Administrator, um Frei… | T1.5 |
...
| 94 | `sogo-mail-read#fehlerbehebung` | „Doppelklicken Sie auf einen Nachrichtenbetreff, um ihn… | T2.11 |
...
| 98 | `sogo-mail-compose#schritt-2-neue-nachricht-beginnen` | Die Schaltfläche „Verfassen“ ist ein Stiftsymbol. Meine… | T2.3 |
| 99 | `sogo-mail-compose#schritt-3-nachricht-adressieren` | Ich wollte ja auch loben. Das hier find ich gut! Spezie… | DONE |
| 100 | `sogo-mail-compose#schritt-5-nachricht-schreiben` | Abgesehen von der vermaledeiten ersten Zelle der Tabell… | T2.3 |
| 101 | `sogo-mail-compose#schritt-6-priorit%C3%A4t-festlegen-optional` | Nope, die Priorität findet man im Drei-Punkte-Menü. Und… | T2.3 |
| 102 | `sogo-mail-compose#schritt-8-als-entwurf-speichern-optional` | Wenn man in „Entwürfe“ auf die Nachricht klickt, kann m… | T2.3 |
| 103 | `sogo-mail-compose#accessibility` | Accessibility, die … Fünfzehnte? | DONE-B2 |
| 104 | `sogo-mail-signatures` |  | DONE-B1 |
| 105 | `sogo-mail-signatures#schritt-1-einstellungen-%C3%B6ffnen` | Entweder das mit der Signatur wird mit dem nächsten SOG… | T2.4 |
| 106 | `sogo-mail-signatures#schritt-2-neue-signatur-hinzuf%C3%BCgen` | Nope, zumindest nicht, wenn es bei dem aktuellen Aufbau… | T2.4 |
| 107 | `sogo-mail-signatures#schritt-3-signatur-schreiben` | Erstens kann man den Text in der Abbildung im Tagmodus … | T5.2 |
| 108 | `sogo-mail-signatures#schritt-4-signaturplatzierung-w%C3%A4hlen` | Aktuell sind das zumindest nicht die Optionen, die man … | T2.4 |
| 109 | `sogo-mail-signatures#teil-2-signatur-manuell-einf%C3%BCgen` | Siehe Punkt direkt obendrüber. | T2.4 |
| 110 | `sogo-mail-signatures#schritt-1-einstellungen-%C3%B6ffnen-1` | Es gibt keinen Reiter „Identitäten“ in der aktuellen Ve… | T2.4 |
| 111 | `sogo-mail-signatures#schritt-3-hilfsidentit%C3%A4t-hinzuf%C3%BCgen` | Kann ich nicht überprüfen… | T2.4 |
| 112 | `sogo-mail-signatures#schritt-4-identit%C3%A4t-beim-verfassen-wechseln` | Aktuell gibt es keinen Dropdown-Pfeil neben meiner E-Ma… | T2.4 |
| 113 | `sogo-mail-signatures#beispiel-gesch%C3%A4ftlich--privat` | Ich finde diese Abbildung, ehrlich gesagt, nicht so hil… | T5.2 |
| 114 | `sogo-mail-signatures#fazit` | Was ist eine „saubere“ Signatur? Meinen Sie eine vollst… | T1.2 |
...
| 118 | `sogo-mail-folder-management#schritt-2-ordner-durchsuchen` | Bei mir gibt es auch noch „Vorlagen“ – was auch immer d… | T1.11 |
...
| 120 | `sogo-mail-folder-management#methode-2-verschieben-button-verwenden` | Das Symbol ist nur ein Pfeil, da ist kein Ordner. Der H… | T2.17 |
...
| 124 | `sogo-mail-reply-forward-delete` | „(…) können Sie auf den Absender antworten (…)“  Man an… | T1.10 |
...
| 132 | `sogo-mail-folders-filters#schritt-3-verschachtelte-unterordner-erstellen` | Immer noch Punkt 102. Und die Abbildung ist mal wieder … | T2.6 |
| 133 | `sogo-mail-folders-filters#schritt-4-nachrichten-in-ordner-verschieben` | Mit Rechtsklick lässt sich nach wie vor nichts tun, auc… | T2.6 |
...
| 136 | `sogo-mail-folders-filters#schritt-2-neuen-filter-erstellen` | Korrekterweise klickt man auf „Filter erstellen“. | T2.6 |
| 137 | `sogo-mail-folders-filters#schritt-3-bedingungen-festlegen` | Tabelle, Spalte 1, Überschrift. Aber jetzt mal Spaß bei… | T2.6 |
| 138 | `sogo-mail-folders-filters#schritt-4-aktionen-festlegen` | Selbes Spiel wie oben: Erstens stimmen die beschriebene… | T2.6 |
| 139 | `sogo-mail-folders-filters#beispielfilter` | Meine Meinung: Nehmen Sie als Beispiel hochauflösende S… | T2.6 |
| 140 | `sogo-mail-folders-filters#filter-funktionieren-nicht` | Wieder nur für die Profis? Würd ich kennzeichnen. Weil … | T2.6 |
| 141 | `sogo-mail-folders-filters#ordner-wird-nicht-angezeigt` | Da ist keine „Schaltfläche Aktualisieren“. | T2.6 |
...
| 146 | `sogo-contacts-add#schritt-3-kontaktinformationen-eingeben` | „Firma“ wird als „Organisation“ angezeigt. „Position“ i… | T1.4 |
| 147 | `sogo-contacts-add#schritt-4-zus%C3%A4tzliche-details-hinzuf%C3%BCgen-optional` | Siehe oben. | T1.4 |
| 148 | `sogo-contacts-add#schritt-5-adressbuch-ausw%C3%A4hlen` | Nee, so heißen die Adressbücher einfach nicht auf der a… | T2.10 |
| 149 | `sogo-contacts-add#gruppe-erstellen` | Da sind keine „Kontaktgruppen“. Ich kann aber über „+“ … | T1.3 |
...
| 151 | `sogo-contacts-add#kontakte-importieren-csvvcard` | Wie bei den E-Mails: es gibt das kein Zahnradsymbol, üb… | T2.10 |
...
| 159 | `sogo-contacts-import-export#schritt-1-kontaktmodul-%C3%B6ffnen` | Zum letzten Mal: die Seitenleiste ist OBEN 😊 | T2.9 |
| 160 | `sogo-contacts-import-export#schritt-1-kontaktmodul-%C3%B6ffnen` | „Klicken Sie auf die Schaltfläche Aktionen (oft ein nac… | T2.9 |
...
| 163 | `sogo-contacts-import-export#exportoptionen` | Siehe oben. | T2.9 |
| 164 | `sogo-contacts-import-export#fehlerbehebung` | Siehe oben. | T2.9 |
