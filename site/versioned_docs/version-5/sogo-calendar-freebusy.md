...
Free/busy data is not public. The grid can only show availability that each
attendee has actively shared — which is why sharing has to go both ways:
...
The one-time prerequisite is mutual sharing: the grid only shows
calendars whose owners have granted access — in both directions.
